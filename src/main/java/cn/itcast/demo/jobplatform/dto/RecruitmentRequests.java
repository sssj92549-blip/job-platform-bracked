...
public final class RecruitmentRequests {
    ...
    public record JobInput(@NotBlank @Size(min = 2, max = 100) String title,
                           ...
                           @NotBlank @Size(max = 10000) String requirements,
    ...
    public record Company(@NotBlank @Size(min = 2, max = 100) String companyName,
                          @Size(max = 100) String industry, @Size(max = 50) String city,
    ...
    public record Confirm(@NotNull @Min(1) Integer expectedVersion,
                          ...
                          @NotNull @Pattern(regexp = EDUCATION) String education,
                          @NotNull @Size(max = 50) List<@NotBlank @Size(max = 100) String> skills,
                          java.util.Map<String, List<String>> optionalSections,
                          @PastOrPresent java.time.LocalDate birthDate, @Min(0) @Max(60) Integer workExperienceYears,
                          @Min(0) @Max(120) Integer age) {
        public Confirm(Integer expectedVersion, String name, String contactPhone, String education, List<String> skills, java.util.Map<String, List<String>> optionalSections) {
            this(expectedVersion, name, contactPhone, education, skills, optionalSections, null, null, null);
        }

        public Confirm(Integer expectedVersion, String name, String contactPhone, String education, List<String> skills) {
            this(expectedVersion, name, contactPhone, education, skills, null);
        }
    }
    ...
    public record Review(@NotNull @Pattern(regexp = "APPROVED|REJECTED") String decision,
    ...
    public record AiInput(@Positive Long applicationId, @Positive Long resumeId,
                          @Min(1) Integer resumeVersion, @Positive Long jobId,
                          @Size(min = 1, max = 2000) String question, @Positive Long previousTaskId) {
