...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (path.startsWith("/api/auth/") || path.equals("/api/users/me"))
            response.setHeader("Cache-Control", "no-store");
        if (Set.of("/api/auth/captcha", "/api/auth/login", "/api/auth/register").contains(path))
            guard.rateLimit(request.getRemoteAddr());
        if (!read) SessionSupport.checkCsrf(request);
        if (path.startsWith("/api/auth/")) return true;
        if (read && (path.equals("/api/jobs") || path.matches("/api/jobs/[0-9]+") || path.matches("/api/companies/[0-9]+") || path.matches("/api/users/[0-9]+/avatar")))
            return true;
        ...
        if (required != null && !required.equals(user.role()))
            throw new BusinessException(HttpStatus.FORBIDDEN, 40301, "当前身份无权访问");
