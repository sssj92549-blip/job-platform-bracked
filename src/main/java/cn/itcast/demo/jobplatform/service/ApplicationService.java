...
public class ApplicationService {
    ...
    public Application own(Long id, Profile p, boolean lock) {
        ...
        if ("JOB_SEEKER".equals(p.getRole())) {
            if (!a.getCandidateId().equals(p.getId())) missing();
        } else if ("COMPANY".equals(p.getRole())) {
            if (!jobs.require(a.getJobId(), false).getCompanyId().equals(p.getId())) missing();
        } else forbidden();
        return a;
    }

    public ObjectNode view(Application a, boolean summary) {
        ObjectNode out = b.view(a, "jobSnapshot");
        JsonNode job = b.read(a.getJobSnapshot()), snapshot = b.read(a.getResumeSnapshot());
        out.put("jobTitle", job.path("title").asText());
        out.put("companyName", job.path("companyName").asText());
        ...
        try {
            if (confirmed.hasNonNull("birthDate"))
                age = java.time.Period.between(java.time.LocalDate.parse(confirmed.path("birthDate").asText()), now().toLocalDate()).getYears();
        } catch (java.time.DateTimeException ignored) {
        }
        out.set("candidateAge", b.json.valueToTree(age));
        if (summary) {
            out.remove("resumeSnapshot");
            String name = snapshot.path("confirmedProfile").path("name").asText();
            if ("AI".equals(a.getProfileSource()) && snapshot.path("aiProfile").hasNonNull("name"))
                name = snapshot.path("aiProfile").path("name").asText();
            out.put("candidateName", name);
        } else {
            ObjectNode s = (ObjectNode) snapshot.deepCopy();
            s.put("downloadUrl", "/api/applications/" + a.getId() + "/resume-file");
            out.set("resumeSnapshot", s);
        }
        return out;
    }
    ...
    public ObjectNode create(Apply input, HttpServletRequest request) {
        ...
        redis.limit(p.getId(), "apply", 30, 60);
        ...
        try {
            return tx.execute(s -> {
                ...
                a.setJobId(j.getId());
                a.setResumeId(r.getId());
                a.setResumeVersion(r.getVersion());
                a.setJobVersion(j.getVersion());
                a.setStatus("SUBMITTED");
                a.setProfileSource("CONFIRMED");
            ...
            throw new BusinessException(HttpStatus.CONFLICT, 40903, "已投递该职位，撤回后也不能重复投递");
        } finally {
    ...
    private void enqueueMatch(Application a, Long companyId) {
        if (matches.selectCount(new QueryWrapper<ResumeJobMatch>().eq("resume_id", a.getResumeId()).eq("resume_version", a.getResumeVersion()).eq("job_id", a.getJobId()).eq("job_version", a.getJobVersion())) > 0)
            return;
    ...
    public PageResult<ObjectNode> list(Map<String, String> q, boolean company, HttpServletRequest request) {
        Profile p = b.actor(request, company ? "COMPANY" : "JOB_SEEKER");
        QueryWrapper<Application> w = new QueryWrapper<>();
        if (company) w.apply("job_id in (select id from job where company_id={0})", p.getId());
        else w.eq("candidate_id", p.getId());
    ...
    private void applyCandidateFilters(QueryWrapper<Application> w, Map<String, String> q) {
        ...
        if (education != null && !education.isBlank()) {
            ...
            w.in("JSON_UNQUOTE(JSON_EXTRACT(resume_snapshot,'$.confirmedProfile.education'))", levels.subList(minimum, levels.size()));
        }
        ...
        if (min != null)
            w.apply("(" + birthDate + "<={0} OR (" + birthDate + " IS NULL AND CAST(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(resume_snapshot,'$.confirmedProfile.age')),'null') AS UNSIGNED)>={1}))", today.minusYears(min).toString(), min);
        if (max != null)
            w.apply("(" + birthDate + ">{0} OR (" + birthDate + " IS NULL AND CAST(NULLIF(JSON_UNQUOTE(JSON_EXTRACT(resume_snapshot,'$.confirmedProfile.age')),'null') AS UNSIGNED)<={1}))", today.minusYears(max + 1L).toString(), max);
    }
    ...
    public ObjectNode change(Long id, String status, String source, HttpServletRequest request) {
        ...
        Application a = own(id, p, true);
        String before = a.getStatus();
        if (source != null) {
            if ("WITHDRAWN".equals(before)) state("已撤回投递不能更改展示来源");
            String previous = a.getProfileSource();
            a.setProfileSource(source);
            ...
            Set<String> allowed = switch (before) {
                ...
                case "SHORTLISTED" -> Set.of("REJECTED", "WITHDRAWN");
                default -> Set.of();
            };
            if (!allowed.contains(status)) state("当前投递状态不允许此操作");
            a.setStatus(status);
        }
    ...
    public ResponseEntity<Resource> download(Long id, HttpServletRequest request) {
        ...
        Resume r = resumes.historical(a.getResumeId());
        if (r == null) missing();
