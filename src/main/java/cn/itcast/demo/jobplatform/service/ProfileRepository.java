...
public class ProfileRepository {
    ...
    public ProfileRepository(ProfileMapper reads,IdentityProfileMapper identities,CandidateProfileMapper candidates,CompanyProfileMapper companies) {
        this.reads=reads; this.identities=identities; this.candidates=candidates; this.companies=companies;
    }
    public Profile selectById(Serializable id) { return reads.selectById(id); }
    public List<Profile> selectList(Wrapper<Profile> query) { return reads.selectList(query); }
    public Profile selectOne(Wrapper<Profile> query) { return reads.selectOne(query); }
    public Page<Profile> selectPage(Page<Profile> page,Wrapper<Profile> query) { return reads.selectPage(page,query); }
    /** 注册新身份与相应资料；管理员无需专属资料表。 */
    @Transactional
    public int insert(Profile profile) {
        IdentityProfile identity=new IdentityProfile(); BeanUtils.copyProperties(profile,identity);
        if(identity.getEnabled()==null) identity.setEnabled(true);
        int count=identities.insert(identity); BeanUtils.copyProperties(identity,profile);
        if("JOB_SEEKER".equals(profile.getRole())) {
            CandidateProfile candidate=new CandidateProfile(); BeanUtils.copyProperties(profile,candidate); candidate.setProfileId(identity.getId());
            if(candidate.getDiscoverable()==null) candidate.setDiscoverable(false); candidates.insert(candidate);
        } else if("COMPANY".equals(profile.getRole())) {
            CompanyProfile company=new CompanyProfile(); BeanUtils.copyProperties(profile,company); company.setProfileId(identity.getId());
            if(company.getReviewStatus()==null) company.setReviewStatus("PENDING"); companies.insert(company);
        }
        return count;
    }
    /** 与MyBatis-Plus默认策略一致，仅更新非null属性。 */
    @Transactional
    public int updateById(Profile patch) { return updateFields(patch.getId(),patch,Collections.emptyMap()); }
    ...
    public int updateFields(Long id,Profile patch,Map<String,Object> explicit) {
        ...
        for(var property:source.getPropertyDescriptors()) {
            ...
            Object value=source.getPropertyValue(name); if(value!=null) target.setPropertyValue(name,value);
        }
        explicit.forEach(target::setPropertyValue);
        identity.setName(current.getName()); identity.setAvatarPath(current.getAvatarPath()); identity.setEnabled(current.getEnabled());
    ...
    public static Map<String,Object> fields(Object... pairs) {
        Map<String,Object> values=new LinkedHashMap<>(); for(int i=0;i<pairs.length;i+=2) values.put((String)pairs[i],pairs[i+1]); return values;
    }
}
