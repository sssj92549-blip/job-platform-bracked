...
public class ResumeService {
    ...
    public Resume confirmed(Long id, Integer version, Long candidate) {
        Resume r = own(id, candidate, true);
        version(Objects.equals(r.getVersion(), version));
        if (!"SUCCESS".equals(r.getParseStatus()) || !"CONFIRMED".equals(r.getConfirmationStatus()))
            state("请先解析并确认当前简历");
        return r;
    }
    ...
    public ObjectNode view(Resume r) {
        ...
        for (String name : List.of("parsedSkills", "conflicts", "confirmedProfile", "parsedWorkExperience", "parsedInternshipExperience", "parsedProjectExperience", "parsedCampusExperience", "parsedCertificates"))
            out.set(name, b.read(out.path(name).isNull() ? null : out.path(name).asText()));
        if (out.path("parsedSkills").isNull()) out.putArray("parsedSkills");
        if (out.path("conflicts").isNull()) out.putArray("conflicts");
        out.put("downloadUrl", "/api/resumes/" + r.getId() + "/file");
        return out;
    }

    public ObjectNode current(HttpServletRequest request) {
        Profile p = b.actor(request, "JOB_SEEKER");
        return view(resumes.selectOne(new QueryWrapper<Resume>().eq("candidate_id", p.getId()).eq("is_current", true)));
    }

    public ObjectNode detail(Long id, HttpServletRequest request) {
        return view(own(id, b.actor(request, "JOB_SEEKER").getId(), false));
    }

    public ObjectNode upload(MultipartFile file, HttpServletRequest request) {
        Profile p = b.actor(request, "JOB_SEEKER");
        redis.limit(p.getId(), "upload", 10, 3600);
        ...
        try {
            ...
            return tx.execute(s -> {
                ...
                if (old != null) {
                    ...
                    resumes.updateById(old);
                    vectors.enqueue(old, true);
                }
                Resume r = new Resume();
                r.setCandidateId(p.getId());
                r.setVersion(1);
                r.setIsCurrent(true);
                r.setDeleted(false);
                r.setFileName(stored.name());
                r.setFilePath(stored.path());
                r.setFileSize(stored.size());
                r.setParseStatus("PENDING");
                r.setConfirmationStatus("UNCONFIRMED");
                r.setConflictStatus("NONE");
                r.setIndexStatus("NOT_READY");
            ...
            redis.unlock(key, token);
        }
    }

    private ObjectNode accepted(Resume r) {
        return b.object("resumeId", r.getId().toString(), "version", r.getVersion(), "parseStatus", r.getParseStatus(), "confirmationStatus", r.getConfirmationStatus(), "indexStatus", r.getIndexStatus());
    }

    @Transactional
    public ObjectNode confirm(Long id, Confirm input, HttpServletRequest request) {
        Profile p = b.actor(request, "JOB_SEEKER");
        Resume r = own(id, p.getId(), true);
        version(r.getVersion().equals(input.expectedVersion()));
        if (!"SUCCESS".equals(r.getParseStatus())) state("解析成功后才可确认");
        vectors.enqueue(r, true);
        r.setVersion(r.getVersion() + 1);
        r.setConfirmationStatus("CONFIRMED");
        ...
        if (input.optionalSections() != null) {
            for (var entry : input.optionalSections().entrySet()) {
                ...
                if (items != null && (items.size() > 20 || items.stream().anyMatch(v -> v == null || v.isBlank() || v.length() > 2000)))
                    bad("每类经历最多20项，每项1至2000字");
                sections.set(entry.getKey(), items == null || items.isEmpty() ? NullNode.instance : b.json.valueToTree(items));
            }
        }
        confirmed.set("age", b.json.valueToTree(input.age()));
        confirmed.put("ageAsOf", now().toLocalDate().toString());
        confirmed.set("optionalSections", sections);
        r.setConfirmedProfile(b.write(confirmed));
        r.setConflictStatus("RESOLVED");
        resumes.updateById(r);
        Profile update = new Profile();
        update.setId(p.getId());
        update.setName(input.name().trim());
        update.setEducation(input.education());
        profiles.updateById(update);
        vectors.enqueue(r, false);
        return view(r);
    }
    ...
    public ObjectNode reparse(Long id, Version input, HttpServletRequest request) {
        Profile p = b.actor(request, "JOB_SEEKER");
        redis.limit(p.getId(), "parse", 10, 3600);
        Resume r = own(id, p.getId(), true);
        version(r.getVersion().equals(input.expectedVersion()));
        ...
        update.setVersion(r.getVersion());
        update.setParseStatus("PENDING");
        update.setConfirmationStatus("UNCONFIRMED");
        update.setIndexStatus("NOT_READY");
        update.setConflictStatus("NONE");
        resumes.update(update, w);
        return accepted(r);
    }
    ...
    public ObjectNode retry(Long id, Version input, HttpServletRequest request) {
        Profile p = b.actor(request, "JOB_SEEKER");
        redis.limit(p.getId(), "index", 10, 3600);
        Resume r = own(id, p.getId(), true);
        version(r.getVersion().equals(input.expectedVersion()));
        if (!"SUCCESS".equals(r.getParseStatus()) || !"FAILED".equals(r.getIndexStatus())) state("仅失败的索引可重试");
        vectors.enqueue(r, false);
        return accepted(r);
    }
    ...
    public ObjectNode delete(Long id, HttpServletRequest request) {
        Resume r = own(id, b.actor(request, "JOB_SEEKER").getId(), true);
        vectors.enqueue(r, true);
    ...
    public ResponseEntity<Resource> download(Long id, HttpServletRequest request) {
        Resume r = own(id, b.actor(request, "JOB_SEEKER").getId(), false);
        return files.download(r.getFilePath(), r.getFileName(), true);
    }
    ...
    public void processOne() {
        ...
        if (r == null) return;
        Resume processing = new Resume();
        processing.setParseStatus("PROCESSING");
        processing.setParseStartedAt(now());
        if (resumes.update(processing, new UpdateWrapper<Resume>().eq("id", r.getId()).eq("version", r.getVersion()).eq("parse_status", "PENDING")) != 1)
            return;
        try {
            ...
            tx.executeWithoutResult(s -> {
                ...
                fresh.setParseStatus("SUCCESS");
                fresh.setExtractedText(result.path("extractedText").asText());
                fresh.setExtractionMethod(result.path("extractionMethod").asText());
                fresh.setPageCount(result.path("pageCount").asInt());
                fresh.setParsedName(text(result, "parsedName"));
                fresh.setParsedPhone(text(result, "parsedPhone"));
                fresh.setParsedEducation(text(result, "parsedEducation"));
                fresh.setParsedSummary(text(result, "parsedSummary"));
                fresh.setParsedSkills(result.path("parsedSkills").toString());
                ...
                if (fresh.getParsedBirthDate() != null && fresh.getParsedBirthDate().isAfter(now().toLocalDate()))
                ...
                ArrayNode conflicts = b.json.createArrayNode();
                JsonNode original = b.read(fresh.getOriginalProfile());
                for (String field : List.of("name", "phone", "education")) {
                    String ai = text(result, "parsed" + Character.toUpperCase(field.charAt(0)) + field.substring(1));
                    String user = text(original, field);
                    if (ai != null && !ai.isBlank() && !Objects.equals(trim(user), trim(ai)))
                        conflicts.add(b.object("field", field, "userValue", user, "aiValue", ai));
                }
                fresh.setConflicts(conflicts.toString());
                fresh.setConflictStatus(conflicts.isEmpty() ? "NONE" : "PENDING_VERIFY");
                resumes.updateById(fresh);
                vectors.enqueue(fresh, false);
            });
        } catch (Exception e) {
            Resume failed = new Resume();
            failed.setParseStatus("FAILED");
            failed.setParseError(e instanceof BusinessException ? e.getMessage() : "简历解析失败，请重试");
    ...
    private static String optionalArray(JsonNode result, String field) {
        ...
        for (JsonNode item : value)
            if (!item.isTextual() || item.asText().isBlank() || item.asText().length() > 2000) PythonAiClient.invalid();
    ...
    private static String text(JsonNode n, String key) {
        return n.hasNonNull(key) ? n.path(key).asText() : null;
    }
    ...
    private static void validateParse(Resume r, JsonNode result) {
        if (!result.path("resumeId").asText().equals(r.getId().toString()) || !result.path("resumeVersion").isIntegralNumber() || result.path("resumeVersion").asInt() != r.getVersion()
                ...
                || !result.path("parsedSkills").isArray() || result.path("parsedSkills").size() > 50)
            PythonAiClient.invalid();
        for (JsonNode skill : result.path("parsedSkills"))
            if (!skill.isTextual() || skill.asText().isBlank() || skill.asText().length() > 100)
                PythonAiClient.invalid();
        for (var field : Map.of("parsedName", 50, "parsedPhone", 32, "parsedSummary", 2000).entrySet()) {
            JsonNode value = result.path(field.getKey());
            if (value.isMissingNode() || (!value.isNull() && (!value.isTextual() || value.asText().length() > field.getValue())))
                PythonAiClient.invalid();
        }
        for (String field : List.of("parsedWorkExperience", "parsedInternshipExperience", "parsedProjectExperience", "parsedCampusExperience", "parsedCertificates"))
            optionalArray(result, field);
        JsonNode education = result.path("parsedEducation");
        if (education.isMissingNode() || (!education.isNull() && !Set.of("HIGH_SCHOOL", "JUNIOR_COLLEGE", "BACHELOR", "MASTER", "DOCTOR", "OTHER").contains(education.asText())))
            PythonAiClient.invalid();
    }
    ...
    public void recover() {
        Resume failed = new Resume();
        failed.setParseStatus("FAILED");
        failed.setParseError("解析任务中断或超时，请重新解析");
