...
public class TalentService {
    ...
    public ObjectNode search(Long id,Talent input,HttpServletRequest request) {
        Profile p=b.actor(request,"COMPANY"); redis.limit(p.getId(),"talent",10,60);
        Job job=jobs.own(id,p,false); if(!"APPROVED".equals(job.getStatus())) state("仅已发布职位可检索人才");
        ...
        ArrayNode refs=b.json.createArrayNode(); Map<String,Integer> versions=new HashMap<>();
        for(Resume r:allowed) { refs.add(b.object("resumeId",r.getId().toString(),"resumeVersion",r.getVersion())); versions.put(r.getId().toString(),r.getVersion()); }
        if(allowed.isEmpty()) return b.object("jobId",id.toString(),"candidates",List.of(),"returnedCount",0);
        int top=input.topK()==null?10:input.topK(); double min=input.minSimilarity()==null?0.6:input.minSimilarity();
        ...
        jobs.publicJob(id); ArrayNode candidates=b.json.createArrayNode(); Set<String> seen=new HashSet<>();
        for(JsonNode m:result.path("matches")) {
            String resumeId=m.path("resumeId").asText(); if(!versions.containsKey(resumeId)||versions.get(resumeId)!=m.path("resumeVersion").asInt()||!seen.add(resumeId)) continue;
            double similarity=m.path("similarity").asDouble(-1); if(!Double.isFinite(similarity)||similarity<min||similarity>1) continue;
            Resume r=resumes.selectOne(eligible(id).eq("id",resumeId).eq("version",versions.get(resumeId))); if(r==null) continue;
