...
public class BusinessSupport {
    ...
    public ObjectNode view(Object entity, String... remove) {
        ...
        for (String name : names) {
            ...
            if (name.endsWith("At") && v.isTextual() && !v.asText().endsWith("Z") && !v.asText().contains("+"))
                node.put(name, v.asText() + "+08:00");
        }
        return node;
    }
    ...
    public static void bad(String message) {
        throw new BusinessException(HttpStatus.BAD_REQUEST, 40001, message);
    }

    public static void state(String message) {
        throw new BusinessException(HttpStatus.CONFLICT, 40902, message);
    }

    public static void missing() {
        throw new BusinessException(HttpStatus.NOT_FOUND, 40401, "资源不存在或不属于当前用户");
    }

    public static void forbidden() {
        throw new BusinessException(HttpStatus.FORBIDDEN, 40301, "当前身份无权执行此操作");
    }

    public static void version(boolean matches) {
        if (!matches) throw new BusinessException(HttpStatus.CONFLICT, 40904, "简历版本已变化，请重新加载");
    }
    ...
    public static int number(Map<String, String> query, String key, int fallback, int min, int max) {
        try {
            int n = Integer.parseInt(query.getOrDefault(key, String.valueOf(fallback)));
    ...
    public static long page(Map<String, String> query) {
        return number(query, "page", 1, 1, 100000);
    }

    public static long size(Map<String, String> query) {
        return number(query, "size", 10, 1, 50);
    }
}
