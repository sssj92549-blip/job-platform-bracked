...
public class AccountService {
    ...
    public AuthViews.Registered register(AuthRequests.Register input) {
        ...
        account.setUsername(input.username().toLowerCase(Locale.ROOT));
        account.setPasswordHash(passwords.encode(input.password()));
        account.setEnabled(true);
        try {
            accounts.insert(account);
        } catch (DuplicateKeyException e) {
            throw new BusinessException(HttpStatus.CONFLICT, 40901, "手机号或用户名已注册");
        }
        Profile p = newProfile(account.getId(), input.role(), input.companyName());
        profiles.insert(p);
    ...
    public Account requireAccount(Long id) {
        ...
        if (a == null || !Boolean.TRUE.equals(a.getEnabled()))
            throw new BusinessException(HttpStatus.UNAUTHORIZED, 40101, "账号已失效，请重新登录");
        return a;
    }
    ...
    public AuthViews.ProfileOption add(Long accountId, AuthRequests.AddProfile input) {
        ...
        try {
            ...
            throw new BusinessException(HttpStatus.CONFLICT, 40906, "已存在该角色身份");
        }
        return option(p);
    }
    ...
    private Profile newProfile(Long accountId, String role, String companyName) {
        ...
        p.setReviewStatus("COMPANY".equals(role) ? "PENDING" : "APPROVED");
        p.setEnabled(true);
        p.setDiscoverable(false);
        return p;
    }

    private void validateRole(String role, String companyName) {
        if (!Set.of("JOB_SEEKER", "COMPANY").contains(role)) bad("不允许注册该角色");
        if ("COMPANY".equals(role) && (companyName == null || companyName.trim().length() < 2 || companyName.trim().length() > 100))
            bad("公司名称须为2到100字");
    }
