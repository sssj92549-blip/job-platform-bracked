...
public class AuthService {
    ...
    public AuthViews.Session login(AuthRequests.Login input, HttpServletRequest request) {
        ...
        synchronized (session) {
            ...
            if (!Boolean.TRUE.equals(account.getEnabled()))
                throw new BusinessException(HttpStatus.FORBIDDEN, 40301, "账号已停用");
            List<Profile> profiles = accounts.profiles(account.getId());
            if (profiles.stream().noneMatch(p -> Boolean.TRUE.equals(p.getEnabled())))
                throw new BusinessException(HttpStatus.FORBIDDEN, 40301, "暂无可用身份");
            guard.succeeded(key);
            session.setAttribute(SessionSupport.ACCOUNT, account.getId());
            session.removeAttribute(SessionSupport.PROFILE);
            if (profiles.size() == 1) {
                session.setAttribute(SessionSupport.PROFILE, profiles.get(0).getId());
    ...
    public AuthViews.Session session(HttpServletRequest request) {
    ...
    private AuthViews.Session choose(AuthRequests.Choose input, HttpServletRequest request, boolean selecting) {
        ...
        synchronized (session) {
            ...
            if (selecting != "SELECT_PROFILE".equals(before.stage()))
                throw new BusinessException(HttpStatus.CONFLICT, 40902, "当前会话状态不允许此操作");
            Long accountId = SessionSupport.account(session);
            if (!selecting) checkProfileHeader(request, session);
            long id;
            try {
                id = Long.parseLong(input.profileId());
            } catch (NumberFormatException e) {
                throw new BusinessException(HttpStatus.BAD_REQUEST, 40001, "身份ID不合法");
            }
            accounts.requireProfile(accountId, id);
            session.setAttribute(SessionSupport.PROFILE, id);
            session.removeAttribute("auth.selectionExpires");
            session.setMaxInactiveInterval(1800);
            SessionSupport.rotate(request);
            return snapshot(session);
        }
    ...
    public AuthViews.ProfileOption add(AuthRequests.AddProfile input, HttpServletRequest request) {
        ...
        synchronized (session) {
            SessionSupport.checkCsrf(request);
            checkProfileHeader(request, session);
            requireUser(request);
    ...
    public AuthViews.Session snapshot(HttpSession session) {
        synchronized (session) {
            ...
            if (accountId == null)
                return new AuthViews.Session("ANONYMOUS", null, List.of(), SessionSupport.csrf(session));
            ...
            if (profileId == null) {
                ...
                if (expiry == null || expiry < System.currentTimeMillis()) {
                    session.invalidate();
                    throw new BusinessException(HttpStatus.UNAUTHORIZED, 40101, "身份选择已过期，请重新登录");
                }
            }
