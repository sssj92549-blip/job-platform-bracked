...
public class ProfileService {
    ...
    public AuthViews.User view(Profile p) {
        return accountService.user(accounts.selectById(p.getAccountId()), p);
    }
    ...
    public AuthViews.User company(Company input, HttpServletRequest request) {
        ...
        Profile update = new Profile();
        update.setCompanyName(input.companyName().trim());
    ...
    public ObjectNode submit(HttpServletRequest request) {
        ...
        Profile update = new Profile();
        update.setReviewStatus("PENDING");
        profiles.updateFields(p.getId(), update, ProfileRepository.fields("reviewReason", null));
        return b.object("reviewStatus", "PENDING");
    }
    ...
    public ObjectNode discover(Discoverability input, HttpServletRequest request) {
        ...
        update.setDiscoverable(input.discoverable());
    ...
    public ObjectNode avatar(MultipartFile file, HttpServletRequest request) {
        Profile p = b.actor(request);
        redis.limit(p.getId(), "avatar", 20, 3600);
        var saved = files.save(file, false);
        Profile update = new Profile();
        update.setId(p.getId());
        update.setAvatarPath(saved.path());
        profiles.updateById(update);
    ...
    public ResponseEntity<Resource> avatar(Long id) {
        ...
        if (p == null || !b.available(p)) missing();
        return files.download(p.getAvatarPath(), "avatar", false);
    }

    public PageResult<AuthViews.User> list(Map<String, String> q, HttpServletRequest request) {
        b.actor(request, "ADMIN");
        QueryWrapper<Profile> w = new QueryWrapper<>();
        for (String key : List.of("role", "reviewStatus"))
            if (q.containsKey(key) && !q.get(key).isBlank())
                w.eq(key.equals("role") ? "role" : "review_status", q.get(key));
        if (q.containsKey("enabled")) {
            if (!Set.of("true", "false").contains(q.get("enabled"))) bad("enabled须为布尔值");
            w.eq("enabled", Boolean.parseBoolean(q.get("enabled")));
        }
        String word = q.get("keyword");
        if (word != null && !word.isBlank())
            w.and(n -> n.like("name", word).or().like("company_name", word).or().apply("account_id in (select id from account where phone like {0} or username like {0})", "%" + word + "%"));
    ...
    public AuthViews.User detail(Long id, HttpServletRequest request) {
        b.actor(request, "ADMIN");
        Profile p = profiles.selectById(id);
        if (p == null) missing();
        return view(p);
    ...
    public AuthViews.User manage(Long id, Review review, Enabled enabled, HttpServletRequest request) {
        Profile actor = b.actor(request, "ADMIN");
        Profile p = profiles.selectOne(new QueryWrapper<Profile>().eq("id", id).last("FOR UPDATE"));
        if (p == null) missing();
        if ("ADMIN".equals(p.getRole())) forbidden();
        ObjectNode before = b.object("reviewStatus", p.getReviewStatus(), "enabled", p.getEnabled());
        ...
        if (review != null) {
            ...
            if ("JOB_SEEKER".equals(p.getRole())) {
                ...
                if (r != null && "SUCCESS".equals(r.getParseStatus()))
                    vectors.enqueue(r, !enabled.enabled() || !Boolean.TRUE.equals(p.getDiscoverable()));
            }
        }
        audit.record(actor.getId(), "PROFILE", id, action, reason, before, b.object("reviewStatus", p.getReviewStatus(), "enabled", p.getEnabled()));
        return view(p);
    }
}
