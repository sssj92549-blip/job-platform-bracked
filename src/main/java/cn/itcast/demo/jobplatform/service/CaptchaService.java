...
public class CaptchaService {
    ...
    public AuthViews.Captcha create(HttpSession session) {
        ...
        try {
            g.setColor(new Color(239,246,236)); g.fillRect(0,0,160,48);
            for(int i=0;i<10;i++) {
            ...
            g.setFont(new Font(Font.SANS_SERIF,Font.BOLD,30)); g.setColor(new Color(30,95,78));
            for(int i=0;i<4;i++) g.drawString(answer.substring(i,i+1),14+i*35,33+random.nextInt(7));
        } finally { g.dispose(); }
        try(ByteArrayOutputStream out=new ByteArrayOutputStream()) {
            ImageIO.write(image,"png",out);
            synchronized(session) { session.setAttribute(KEY,new Challenge(id,answer.toString(),System.currentTimeMillis()+120000)); }
        ...
        } catch(IOException e) { throw new IllegalStateException("验证码生成失败",e); }
    }
    /** 无论答对与否都消费验证码，防止对同一图片连续猜测。 */
    public void verify(HttpSession session,String id,String answer) {
        synchronized(session) {
            Challenge c=(Challenge)session.getAttribute(KEY); session.removeAttribute(KEY);
