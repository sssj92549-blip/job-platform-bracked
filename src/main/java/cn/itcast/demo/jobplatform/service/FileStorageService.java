...
public class FileStorageService {
    ...
    public FileStorageService(@Value("${app.storage.root}") String root) {
    ...
    public Stored save(MultipartFile file, boolean pdf) {
        ...
        if (file.getSize() > (pdf ? 10485760 : 2097152))
            throw new BusinessException(HttpStatus.PAYLOAD_TOO_LARGE, 41301, "上传文件过大");
        try {
            ...
            if (pdf) {
                ...
                try (var document = Loader.loadPDF(bytes)) {
                    if (document.isEncrypted() || document.getNumberOfPages() < 1 || document.getNumberOfPages() > 20)
                        bad("PDF须为未加密的1至20页文档");
                }
                extension = "pdf";
            } else {
                try (var imageInput = ImageIO.createImageInputStream(new ByteArrayInputStream(bytes))) {
                    var readers = ImageIO.getImageReaders(imageInput);
                    if (!readers.hasNext()) bad("仅支持JPEG或PNG图片");
                    var reader = readers.next();
                    try {
                        String format = reader.getFormatName().toLowerCase(Locale.ROOT);
                        if (!Set.of("jpeg", "png").contains(format)) bad("仅支持JPEG或PNG图片");
                        reader.setInput(imageInput);
                        if ((long) reader.getWidth(0) * reader.getHeight(0) > 16000000) bad("图片像素过大");
                        var decoded = reader.read(0);
                        ByteArrayOutputStream clean = new ByteArrayOutputStream();
                        ImageIO.write(decoded, format, clean);
                        bytes = clean.toByteArray();
                        extension = format;
                    } finally {
                        reader.dispose();
                    }
            ...
            String name = Optional.ofNullable(file.getOriginalFilename()).orElse("resume.pdf").replace('\\', '/');
            ...
            throw new BusinessException(HttpStatus.BAD_REQUEST, 40001, "文件不可读或存储失败");
        }
    }
