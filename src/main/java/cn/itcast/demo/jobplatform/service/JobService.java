...
public class JobService {
    ...
    public Job own(Long id, Profile p, boolean lock) {
        Job j = require(id, lock);
        if (!j.getCompanyId().equals(p.getId())) missing();
        return j;
    }

    public ObjectNode view(Job j, boolean publicView) {
        ObjectNode out = b.view(j, "deleted");
        out.set("skills", b.read(j.getSkills()));
        companyFields(out, profiles.selectById(j.getCompanyId()));
        if (publicView) out.putNull("reviewReason");
        else {
            var index = vectors.latest(j.getId());
            out.put("indexStatus", index.isEmpty() ? "NOT_READY" : String.valueOf(index.get("status")));
            out.put("indexError", (String) index.get("error_message"));
        }
        return out;
    }
    ...
    public ObjectNode detail(Long id) {
        Job j = publicJob(id);
        String cached = redis.cachedJob(id, j.getVersion());
        ObjectNode out = cached == null ? view(j, true) : (ObjectNode) b.read(cached);
        companyFields(out, profiles.selectById(j.getCompanyId()));
        if (cached == null) redis.cacheJob(id, j.getVersion(), b.write(out));
        return out;
    }
    ...
    public PageResult<ObjectNode> list(Map<String, String> q, String scope, HttpServletRequest request) {
        ...
        if ("PUBLIC".equals(scope)) {
            ...
            if (q.containsKey("companyId") && !q.get("companyId").isBlank()) {
                try {
                    long companyId = Long.parseLong(q.get("companyId"));
                    if (companyId <= 0) throw new NumberFormatException();
        ...
        if (!"PUBLIC".equals(scope) && keyword != null && !keyword.isEmpty())
            w.and(n -> n.like("title", keyword).or().apply("company_id in (select id from profile_details where company_name like {0})", "%" + keyword + "%"));
        ...
        if (q.containsKey("education") && !q.get("education").isBlank()) {
            ...
            if ("OTHER".equals(education)) w.eq("education_requirement", "OTHER");
                ...
                int minimum = levels.indexOf(education);
                if (minimum < 0) bad("学历筛选无效");
        ...
        if (q.containsKey("companySize") && !q.get("companySize").isBlank()) {
            if (!Set.of("UNDER_20", "20_99", "100_499", "500_999", "1000_9999", "10000_PLUS").contains(q.get("companySize")))
                bad("公司规模无效");
        ...
        if (q.containsKey("salaryMin") && q.containsKey("salaryMax") && Integer.parseInt(q.get("salaryMin")) > Integer.parseInt(q.get("salaryMax")))
            bad("薪资下限不能大于上限");
        if ("PUBLIC".equals(scope) && "recommended".equals(q.get("mode"))) return recommended(q, w, request, keyword);
        if ("PUBLIC".equals(scope) && ((keyword != null && !keyword.isBlank()) || (q.get("industry") != null && !q.get("industry").isBlank())))
            return hybrid(q, w, keyword);
    ...
    private PageResult<ObjectNode> recommended(Map<String, String> q, QueryWrapper<Job> w, HttpServletRequest request, String keyword) {
        ...
        if (keyword != null && !keyword.isBlank()) {
            ...
            scores.keySet().retainAll(rank(keyword, allowed, true).keySet());
        }
        Map<Long, Integer> versions = new HashMap<>();
        for (Job j : allowed) versions.put(j.getId(), j.getVersion());
    ...
    private Map<Long, Double> rank(String query, List<Job> eligible, boolean includeCompany) {
        ...
        for (Job j : eligible) {
            ...
            if (includeCompany) {
                ...
                if (company != null) content += " " + company.getCompanyName();
            }
            if (content.toLowerCase(Locale.ROOT).contains(needle))
                scores.merge(j.getId(), title.equals(needle) ? 3.0 : title.contains(needle) ? 2.0 : 1.0, Double::sum);
        }
        return scores;
    }
    ...
    public ObjectNode managedDetail(Long id, String scope, HttpServletRequest request) {
        Profile p = b.actor(request, scope);
        return view("ADMIN".equals(scope) ? require(id, false) : own(id, p, false), false);
    }

    @Transactional
    public ObjectNode save(Long id, JobInput input, HttpServletRequest request) {
        Profile p = b.actor(request, "COMPANY");
        if (!b.available(p)) forbidden();
        ...
        j.setTitle(input.title().trim());
        j.setCity(input.city().trim());
        j.setSalaryMin(input.salaryMin());
        j.setSalaryMax(input.salaryMax());
        j.setEducationRequirement(input.educationRequirement());
        j.setExperienceMinYears(input.experienceMinYears());
        j.setDescription(input.description().trim());
        j.setRequirements(input.requirements().trim());
        j.setSkills(b.write(input.skills()));
        j.setStatus("DRAFT");
        j.setReviewReason(null);
        if (id == null) jobs.insert(j);
        else
            jobs.update(j, new com.baomidou.mybatisplus.core.conditions.update.UpdateWrapper<Job>().eq("id", id).set("education_requirement", input.educationRequirement()).set("review_reason", null));
        return view(j, false);
    }
    ...
    public ObjectNode transition(Long id, String action, Review review, String reason, HttpServletRequest request) {
        ...
        Profile p = b.actor(request, admin ? "ADMIN" : "COMPANY");
        Job j = admin ? require(id, true) : own(id, p, true);
        String before = j.getStatus();
    ...
    private void requireCompanyInfo(Profile p) {
        ...
        if (p.getCompanySize() == null || !Set.of("UNDER_20", "20_99", "100_499", "500_999", "1000_9999", "10000_PLUS").contains(p.getCompanySize()))
            missing.add("公司规模");
    ...
    public ObjectNode delete(Long id, HttpServletRequest request) {
        Job j = own(id, b.actor(request, "COMPANY"), true);
        if (!Set.of("DRAFT", "REJECTED").contains(j.getStatus()) || applications.selectCount(new QueryWrapper<Application>().eq("job_id", id)) > 0)
            state("该职位不能删除");
        vectors.enqueue(j, true);
        jobs.deleteById(id);
        redis.evictJob(id, j.getVersion());
        return b.object();
    }
    ...
    public ObjectNode company(Long id) {
        Profile p = profiles.selectById(id);
        if (p == null || !"COMPANY".equals(p.getRole()) || !b.available(p)) missing();
