...
public class JobVectorService {
    ...
    public void reconcile() {
        for (Job candidate : jobs.selectList(new QueryWrapper<Job>()))
    ...
    public void processOne() {
        ...
        var t = rows.get(0);
        Object id = t.get("id");
        if (db.update("UPDATE job_vector_task SET status='PROCESSING',started_at=?,attempts=attempts+1 WHERE id=? AND status='PENDING'", now(), id) != 1)
            return;
        try {
            ...
            if (response == null || !response.path(delete ? "deleted" : "indexed").asBoolean(false))
     ...
     */
    public Map<Long, Double> search(String keyword, List<Job> allowed) {
        return search(keyword, allowed, 0.55);
    }

    public Map<Long, Double> search(String keyword, List<Job> allowed, double minimum) {
        return search(keyword, allowed, minimum, null, false);
    }

    public Map<Long, Double> recommend(String question, String resumeText, List<Job> allowed) {
        return search(question, allowed, 0.65, resumeText, true);
    }

    /**
     * 对话只用相似度排序，不让旧简历阈值提前截掉转行候选；条件交给通用审核。
     */
    public Map<Long, Double> conversationRank(String query, String resumeText, List<Job> allowed) {
        return search(query, allowed, 0.0, resumeText, true, 0.25);
    }

    private Map<Long, Double> search(String keyword, List<Job> allowed, double minimum, String resumeText, boolean required) {
        return search(keyword, allowed, minimum, resumeText, required, 0.7);
    }

    private Map<Long, Double> search(String keyword, List<Job> allowed, double minimum, String resumeText, boolean required, double resumeWeight) {
        Map<Long, Double> found = new HashMap<>();
        if (allowed.isEmpty()) return found;
        var refs = b.json.createArrayNode();
        Map<Long, Integer> versions = new HashMap<>();
        for (Job j : allowed) {
            refs.add(b.object("jobId", j.getId().toString(), "jobVersion", j.getVersion()));
            versions.put(j.getId(), j.getVersion());
        }
        try {
            var payload = b.object("queryText", keyword, "eligibleJobs", refs, "minSimilarity", minimum);
            if (resumeText != null) {
                payload.put("resumeText", resumeText);
                payload.put("resumeWeight", resumeWeight);
            }
            JsonNode result = python.call(HttpMethod.POST, "/internal/vector/jobs/search", payload);
            if (result == null || !result.path("matches").isArray()) {
                if (required) PythonAiClient.invalid();
                return found;
            }
            for (JsonNode item : result.path("matches")) {
                Long id = Long.valueOf(item.path("jobId").asText());
                double score = item.path("similarity").asDouble(-1);
                if (Objects.equals(versions.get(id), item.path("jobVersion").asInt()) && Double.isFinite(score) && score >= minimum && score <= 1)
                    found.put(id, score);
            }
        } catch (Exception e) {
            if (required)
                throw new cn.itcast.demo.jobplatform.common.BusinessException(org.springframework.http.HttpStatus.SERVICE_UNAVAILABLE, 50301, "职位推荐服务暂不可用，请稍后重试");
            org.slf4j.LoggerFactory.getLogger(getClass()).warn("Job vector search unavailable; using keyword search");
        }
        return found;
    }
}
