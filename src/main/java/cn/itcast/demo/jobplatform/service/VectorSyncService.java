...
public class VectorSyncService {
    ...
    public void enqueue(Resume r, boolean delete) {
        VectorSyncTask t = new VectorSyncTask();
        t.setResumeId(r.getId());
        t.setResumeVersion(r.getVersion());
        t.setOperation(delete ? "DELETE" : "UPSERT");
        t.setStatus("PENDING");
        t.setAttempts(0);
        ...
        tasks.insert(t);
        r.setIndexStatus(delete ? "DELETING" : "PENDING");
        r.setIndexError(null);
    ...
    public void processOne() {
        ...
        if (tasks.update(t, new UpdateWrapper<VectorSyncTask>().eq("id", t.getId()).eq("status", "PENDING")) != 1)
            return;
        try {
            Resume current = resumes.selectById(t.getResumeId());
            Profile p = current == null ? null : profiles.selectById(current.getCandidateId());
            ...
            tx.executeWithoutResult(s -> {
                ...
                update.setIndexStatus(delete ? "DELETED" : "READY");
            ...
            tx.executeWithoutResult(s -> {
                ...
                t.setStatus(t.getAttempts() < 3 ? "PENDING" : "FAILED");
                t.setErrorMessage(message);
                t.setNextAttemptAt(now().plusSeconds(30L * t.getAttempts()));
                tasks.updateById(t);
                if ("FAILED".equals(t.getStatus())) {
                    Resume update = new Resume();
                    update.setIndexStatus("FAILED");
                    update.setIndexError(message);
                    resumes.update(update, new UpdateWrapper<Resume>().eq("id", t.getResumeId()).eq("version", t.getResumeVersion()));
                }
            });
        }
