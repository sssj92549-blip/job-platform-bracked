...
public class ResumeController {
    ...
    public ResponseEntity<?> upload(@RequestParam MultipartFile file, HttpServletRequest request) {
        return ResponseEntity.status(202).body(ApiResponse.success(service.upload(file, request)));
    }

    @GetMapping("/api/resumes/current")
    public ResponseEntity<?> current(HttpServletRequest request) {
        return ResponseEntity.status(200).body(ApiResponse.success(service.current(request)));
    }
    ...
    public ResponseEntity<?> confirm(@PathVariable Long id, @Valid @RequestBody Confirm input, HttpServletRequest request) {
        return ResponseEntity.status(200).body(ApiResponse.success(service.confirm(id, input, request)));
    }

    @PostMapping("/api/resumes/{id}/reparse")
    public ResponseEntity<?> reparse(@PathVariable Long id, @Valid @RequestBody Version input, HttpServletRequest request) {
        return ResponseEntity.status(202).body(ApiResponse.success(service.reparse(id, input, request)));
    }

    @PostMapping("/api/resumes/{id}/retry-index")
    public ResponseEntity<?> retryIndex(@PathVariable Long id, @Valid @RequestBody Version input, HttpServletRequest request) {
        return ResponseEntity.status(202).body(ApiResponse.success(service.retry(id, input, request)));
    }
